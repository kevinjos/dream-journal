...
const createWordCloud = async (): Promise<void> => {
  ...
  const getRandomTextColor = () => textColors[Math.floor(Math.random() * textColors.length)] || secondaryColor
  ...
  const textElements = zoomGroup.selectAll('text')
    .data(nodes)
    .enter()
    .append('text')
    .text(d => d.name)
  ...
  const simulation = d3.forceSimulation(nodes)
    .force('collision', d3.forceCollide<QualityNode>().radius(d => fontScale(d.frequency) / 2 + 8))
