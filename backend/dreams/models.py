...
class QualityGraph:
    ...
    def add_node(self, quality_id: int, quality_name: str, frequency: int) -> None:
        ...
        if quality_id not in self.nodes:
            self.nodes[quality_id] = QualityGraphNode(quality_id, quality_name, frequency)
        ...
        for node in self.nodes.values():
            ...
            for other_id, dream_ids in node.edges.items():
                if other_id in self.nodes:
                    ...
                    connections.append(QualityConnection(
                        quality_id=other_id,
...
class Quality(models.Model):
    ...
    frequency = models.PositiveIntegerField(
        default=1,
        help_text='Number of dreams this quality appears in'
    )
...
class Dream(models.Model):
    ...
    description = models.TextField(
        blank=True,
        help_text='Description of the dream'
    )
