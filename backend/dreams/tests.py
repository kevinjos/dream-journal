...
class SecurityTestCase(APITestCase):
    ...
    def setUp(self) -> None:
        ...
        self.dream_a = Dream.objects.create(
            user=self.user_a, description="I was flying over water"
        )
        self.dream_a.qualities.add(self.quality_a1, self.quality_a2)

        self.dream_b = Dream.objects.create(
            user=self.user_b, description="Had a nightmare"
        )
...
class QualityGraphTestCase(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.dream1 = Dream.objects.create(
            user=self.user, description="Flying over water"
        )
        self.dream1.qualities.add(self.flying, self.water)

        self.dream2 = Dream.objects.create(
            user=self.user, description="Lucid flying dream"
        )
        self.dream2.qualities.add(self.flying, self.lucid)

        self.dream3 = Dream.objects.create(
            user=self.user, description="Swimming lucidly"
        )
    ...
    def test_build_quality_graph(self) -> None:
        ...
        flying_node = next(
            node for node in graph.nodes.values() if node.quality_name == "flying"
        )
...
class NestedRoutesSecurityTestCase(APITestCase):
    ...
    def setUp(self) -> None:
        ...
        self.dream_a = Dream.objects.create(
            user=self.user_a, description="Flying dream"
        )
    ...
    def test_user_cannot_add_quality_to_other_user_dream(self) -> None:
        ...
        response = self.client.put(
            f"/api/dreams/{self.dream_b.pk}/qualities/{self.quality_a.pk}/"
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_user_cannot_add_other_user_quality_to_own_dream(self) -> None:
        ...
        response = self.client.put(
            f"/api/dreams/{self.dream_a.pk}/qualities/{self.quality_b.pk}/"
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_user_can_add_own_quality_to_own_dream(self) -> None:
        ...
        response = self.client.put(
            f"/api/dreams/{self.dream_a.pk}/qualities/{self.quality_a.pk}/"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_user_cannot_remove_quality_from_other_user_dream(self) -> None:
        ...
        response = self.client.delete(
            f"/api/dreams/{self.dream_b.pk}/qualities/{self.quality_b.pk}/"
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
