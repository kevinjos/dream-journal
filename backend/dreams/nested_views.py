...
class DreamQualityViewSet(viewsets.ViewSet):
    ...
    def get_dream(self, dream_id: int | str) -> Dream:
        """Get dream owned by current user or raise 404."""
        user = self.request.user
        if not isinstance(user, User):
            raise PermissionError("Authentication required")

        return get_object_or_404(Dream.objects.filter(user=user), pk=dream_id)

    def get_quality(self, quality_id: int | str) -> Quality:
        """Get quality owned by current user or raise 404."""
        user = self.request.user
        if not isinstance(user, User):
            raise PermissionError("Authentication required")

        return get_object_or_404(Quality.objects.filter(user=user), pk=quality_id)

    def list(self, request: Request, dream_pk: int | str | None = None) -> Response:
        """List all qualities for a specific dream."""
        if dream_pk is None:
            return Response(
                {"error": "Dream ID is required"}, status=status.HTTP_400_BAD_REQUEST
            )

        dream = self.get_dream(dream_pk)
        qualities = dream.qualities.all().order_by("name")
        serializer = QualitySerializer(
            qualities, many=True, context={"request": request}
        )
    ...
    def update(
        ...
        if pk is None:
            return Response(
                {"error": "Quality ID is required"}, status=status.HTTP_400_BAD_REQUEST
            )
