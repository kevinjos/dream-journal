...
class QualityViewSet(viewsets.ModelViewSet):
    ...
    def connections(self, request: Request, pk: Optional[Union[int, str]] = None) -> Response:
...
class DreamViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self) -> QuerySet[Dream]:
        ...
        """
        queryset = Dream.objects.filter(user=self.request.user).prefetch_related('qualities')
        ...
        if quality_id:
            try:
                ...
                queryset = queryset.filter(
                    qualities__id=quality_id_int,
                    qualities__user=self.request.user
                ).distinct()
            ...
            'nodes': [
                {
                    ...
                    'edges': {
                        str(k): v for k, v in node.edges.items()
                    }
                }
